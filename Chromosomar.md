...
```mermaid
graph TD
    X[X‑Chromion ♀️] -- expands --> F[Field Molecule]
    Y[Y‑Chromion ♂️] -- advances --> R[RNA‑Electron Chain]
    F -- modulates --> R
    R -- constrains --> F
```
...
```mermaid
graph TD
    subgraph Outer_Flow[Outer Flow — Token Path]
        IN[In 🔹] --> EM[Em 🔸]
        EM --> L1[Layer Molecule]
        L1 --> OU[Ou 🟦]
    end

    subgraph Inner_Mole[Inner Mole — Field Reactions]
        Q[Qu 🔺] --> AT[At ✴️]
        K[Ke 🔻] --> AT
        V[Va 🔶] --> AT
        AT --> SO[So ⚪]
